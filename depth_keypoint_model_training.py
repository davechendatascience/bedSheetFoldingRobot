...
# Import the fixed threshold function
from src.utils import thresholded_locations
...
def main_training_pipeline(
    ...
    if config.get("use_quantization", False):
        ...
        # Load the pretrained weights into the quantized model BEFORE preparing for QAT
        try:
            print(f"Loading pretrained weights from {config['pretrained_path']}")
            pretrained_state = torch.load(config["pretrained_path"], map_location=device)
            
            # Try to load weights with strict=False to handle structural differences
            missing_keys, unexpected_keys = model.load_state_dict(pretrained_state, strict=False)
            print(f"Pretrained weights loaded into quantized model")
            if missing_keys:
        ...
        # Now prepare the model for QAT after loading weights
    ...
    if config.get("use_quantization", False):
        ...
        qat_lr = config.get("qat_learning_rate", 5e-5)  # Increased from 2e-5
        optimizer = optim.AdamW(
            model.parameters(), 
            lr=qat_lr, 
        ...
        qat_epochs = config.get("qat_epochs", 100)
        
        def qat_lr_lambda(epoch):
            # Stage 1: Warmup (0 to 10)
            if epoch < 10:
                return float(epoch) / 10.0
            
            # Stage 2: High learning rate plateau (10 to 40)
            elif epoch < 40:
                return 1.0
            
            # Stage 3: Gradual decay (40 to 80)
            elif epoch < 80:
                progress = float(epoch - 40) / 40.0
                return 1.0 - 0.6 * progress
            
            # Stage 4: Fine-tuning with very low LR (80 to end)
            else:
                progress = float(epoch - 80) / float(max(1, qat_epochs - 80))
